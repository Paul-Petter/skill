<!-- BKUI-KNOWLEDGE-MANAGED:86f264fd02d7 -->
---
id: bkui-quick-start
name: BKUI 快速入门
category: engineering
description: 蓝鲸前端知识库入口指南，包含规范、索引和工作流程
tags: [bkui, 规范, 索引, 入门, vue3, cli]
...
- **事实源**: Vue3 组件 API / Demo 优先使用 `@bkui-vue/cli`；MCP `get_component_api` 仅兜底
...
1. **分析需求** → 确定需要哪些资源
2. **组件 API / Demo** → 先读 `get_skill({ skillId: 'bkui-vue' })`，再用 `bkui-vue info/demo/snippet`
3. **CLI 不可用** → 按 `bkui-vue` skill 引导安装；安装失败或用户暂不安装时再用 `get_component_api`
4. **模板代码** → `get_skill({ skillId: 'bkui-builder' })`
...
- `./references/skills-index.md` - 完整 Skills 索引
- `./references/components-list.md` - 组件完整列表
